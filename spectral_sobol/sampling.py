...
class TorchSobolSequence(Sampler):
    """
    PyTorch Sobol LP tau sequence sampler.
    ...
    def __call__(self, dimension, nb_design):
        soboleng = torch.quasirandom.SobolEngine(dimension=dimension*2)
        ab = np.array(soboleng.draw(nb_design))
        a, b = ab[:, :dimension], ab[:, dimension:]
        c = self._build_replicated_design(a, b)
