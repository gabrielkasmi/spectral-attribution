...
from .torch_perturbations import inpainting, wavelet, fourier
from .utils import *
import cv2
...
class FourierSobol:
    ...
    def __call__(self, inputs, labels):
        ...
        for input, label in zip(inputs, labels):
            ...
            sti_dispersion = compute_dispersion(sti_components)
            self.dispersion.append(sti_dispersion)
    ...
    def _batch_forward(model, fourier_transform, masks, perturbator, input_shape):
        ...
        perturbed_images = torch.stack([transforms(im) for im in perturbated_inputs]).cuda()
